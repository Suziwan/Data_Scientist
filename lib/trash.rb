...
# Fonction .grep de correction de Océane
arr.grep(/^@[A-Z]/).count

# Pour régler le problème du float dans crypto (Ghislain)
rate_float = rate.map { |value| value.to_f }
link = currency.zip(rate_float)

# Question 8 de Clément
a = 0 
...
distribution(journalist_database)